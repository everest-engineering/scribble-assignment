...
- [x] T001 Review `specs/004-result-restart-validation/plan.md`, `spec.md`, and `contracts/rooms-api.md` against acceptance criteria
- [x] T002 Confirm Scenario 3 flows (draw, guess, score, game poll) work on branch `004-result-restart-validation`
...
- [x] T003 Add `"result"` to `RoomStatus` type in `backend/src/models/game.ts`
- [x] T004 [P] Mirror `RoomStatus` with `"result"` in `frontend/src/services/api.ts`
- [x] T005 Update `toRoomSnapshot()` to expose `secretWord` to all viewers, include `guesses`/scores, and omit `strokes` when `status === "result"` in `backend/src/services/roomStore.ts`
- [x] T006 Update `participantRole()` to return drawer/guesser roles when `status === "result"` in `backend/src/services/roomStore.ts`
- [x] T007 Verify `addStroke`, `clearCanvas`, and `submitGuess` reject when `status !== "playing"` in `backend/src/services/roomStore.ts`
...
- [x] T008 [US1] Implement `endRoom(code, participantId)` with host-only and `playing` guards in `backend/src/services/roomStore.ts`
- [x] T009 [P] [US1] Add `endRoomSchema` (participantId body) in `backend/src/api/schemas.ts`
- [x] T010 [US1] Add `POST /rooms/:code/end` route with error mapping in `backend/src/api/rooms.ts`
- [x] T011 [P] [US1] Add `endRoom` API method in `frontend/src/services/api.ts`
- [x] T012 [US1] Add `endRoom` action to `frontend/src/state/roomStore.ts`
- [x] T013 [US1] Extend poll effect to run when `room.status` is `"playing"` or `"result"` in `frontend/src/pages/GamePage.tsx`
- [x] T014 [US1] Add host-only End Round button and `roomStore.endRoom` handler in `frontend/src/pages/GamePage.tsx`
- [x] T015 [P] [US1] Add vitest cases for endRoom host guard, not_playing reject, and `playing` → `result` transition in `backend/src/services/roomStore.test.ts`
...
- [x] T016 [US2] Branch `GamePage` layout for result mode — hide `DrawingCanvas`, Clear button, and `GuessForm` when `status === "result"` in `frontend/src/pages/GamePage.tsx`
- [x] T017 [US2] Show secret word card to all participants (not drawer-only) when `status === "result"` in `frontend/src/pages/GamePage.tsx`
- [x] T018 [US2] Update header/status copy for result mode (e.g., round ended) in `frontend/src/pages/GamePage.tsx`
- [x] T019 [P] [US2] Add vitest asserting `secretWord` present for non-drawer viewer when `status === "result"` in `backend/src/services/roomStore.test.ts`
- [x] T020 [P] [US2] Add vitest asserting `strokes` omitted from snapshot when `status === "result"` in `backend/src/services/roomStore.test.ts`
...
- [x] T021 [US3] Implement `restartRoom(code, participantId)` with host-only and `result` guards plus round-field clearing in `backend/src/services/roomStore.ts`
- [x] T022 [P] [US3] Add `restartRoomSchema` (participantId body) in `backend/src/api/schemas.ts`
- [x] T023 [US3] Add `POST /rooms/:code/restart` route with error mapping in `backend/src/api/rooms.ts`
- [x] T024 [P] [US3] Add `restartRoom` API method in `frontend/src/services/api.ts`
- [x] T025 [US3] Add `restartRoom` action to `frontend/src/state/roomStore.ts`
- [x] T026 [US3] Add host-only Restart button and `roomStore.restartRoom` handler when `status === "result"` in `frontend/src/pages/GamePage.tsx`
- [x] T027 [US3] Navigate to `/lobby` when polled snapshot shows `status === "lobby"` in `frontend/src/pages/GamePage.tsx`
- [x] T028 [P] [US3] Add vitest cases for restartRoom host guard, not_result reject, and field clearing in `backend/src/services/roomStore.test.ts`
...
- [x] T029 [US4] Verify `startRoom()` after restart re-initializes fresh round state (scores 0, empty history) in `backend/src/services/roomStore.test.ts`
- [x] T030 [US4] Confirm non-host cannot invoke end/restart via API (403) in `backend/src/services/roomStore.test.ts`
...
- [x] T031 Run manual two-browser validation per `specs/004-result-restart-validation/quickstart.md` (full four-scenario loop)
- [x] T032 [P] Run `npm run build` in `backend/` and `frontend/` and fix any type errors
