...
- [ ] T001 Define `"result"` status and optional/nullable `correctGuesserId?: string | null` in `backend/src/models/game.ts`
- [ ] T002 Update Zod schemas in `backend/src/api/schemas.ts` to add `"RESTART_REQUIRES_HOST"`, `"GAME_NOT_IN_RESULT"`, and `"GAME_ALREADY_ENDED"` to `errorCodeSchema`, `"result"` to `roomStatusSchema`, and `correctGuesserId` to `roomSnapshotSchema`
...
- [ ] T004 [P] [US1] Create Zod schema tests in `backend/src/api/schemas.test.ts` verifying Zod parses `"result"` status and nullable/optional `correctGuesserId` successfully in `roomSnapshotSchema`
- [ ] T005 [P] [US1] Create API tests in `frontend/src/services/api.test.ts` covering updated snapshot mapping for results state

### Implementation for User Story 1

- [ ] T006 [US1] Implement single source of truth guard and correct guess transition inside backend `submitGuess` function in `backend/src/services/roomStore.ts` (reject guesses with error `"GAME_ALREADY_ENDED"` if status === `"result"`, transition status to `"result"`, and set `correctGuesserId` only on first correct guess)
- [ ] T007 [US1] Update backend `toRoomSnapshot` in `backend/src/services/roomStore.ts` to unmask/reveal `secretWord` to all players when status is `"result"`, include `correctGuesserId`, and ensure `correctGuesserId` is `null`/`undefined` when status is `"lobby"`
- [ ] T008 [P] [US1] Update `RoomSnapshot` and `Participant` interfaces in `frontend/src/services/api.ts` to support `"result"` status and `correctGuesserId`
- [ ] T010 [US1] Modify `GamePage.tsx` in `frontend/src/pages/GamePage.tsx` to:
...
- [ ] T012 [P] [US2] Create endpoint validation tests in `backend/src/api/schemas.test.ts` for `/restart` schema payload validation and error codes
- [ ] T013 [P] [US2] Create store tests in `frontend/src/state/roomStore.test.ts` verifying `restartRoom` action updates store snapshot and triggers lobby redirect updates

### Implementation for User Story 2

- [ ] T014 [P] [US2] Register `restartRoomSchema` in Zod schemas in `backend/src/api/schemas.ts` (validating request body `{ participantId }`)
- [ ] T015 [US2] Implement backend service `restartRoom(code, participantId)` inside `backend/src/services/roomStore.ts` to atomically clear round state in memory
- [ ] T016 [US2] Register backend route `POST /rooms/:code/restart` in `backend/src/api/rooms.ts` and handle restart errors (`ROOM_NOT_FOUND`, `RESTART_REQUIRES_HOST`, `GAME_NOT_IN_RESULT`)
- [ ] T017 [P] [US2] Add `restartRoom(code, participantId)` API client method in `frontend/src/services/api.ts`
- [ ] T018 [US2] Add `restartRoom` action in `frontend/src/state/roomStore.ts`
- [ ] T019 [US2] Update `GamePage.tsx` in `frontend/src/pages/GamePage.tsx` to:
...
- [ ] T020 Run manual verification steps in `specs/004-result-restart-validation/quickstart.md` using two browser instances
