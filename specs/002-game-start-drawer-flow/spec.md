...
**Given** a game is starting and player names are trimmed with empty or whitespace-only names rejected with a message, **When** the first round begins, **Then** the host or first player becomes the clearly identified drawer, and the secret word is deterministically selected from the starter list and visible only to the drawer.
...
When the game starts, the host or first player becomes the drawer and one secret word is selected deterministically.

**Why this priority**: Deterministic assignment makes behavior predictable, testable, and simple for the lab scope.

**Independent Test**: Start the same valid room flow and verify the host or first participant is the drawer and the first starter word is selected.

**Acceptance Scenarios**:

1. **Given** the host starts a valid game, **When** the round is initialized, **Then** exactly one drawer is assigned and clearly identified.
2. **Given** the host is the first player in the room, **When** the round is initialized, **Then** the host is assigned as drawer.
3. **Given** a room implementation treats the first player as drawer, **When** the first round begins, **Then** the first player is the drawer.
4. **Given** the round is initialized, **When** the secret word is selected, **Then** the selected word comes from the starter word list using a deterministic rule.
...
- **FR-004**: System MUST assign exactly one clearly identified drawer when the game starts.
- **FR-005**: System MUST assign the host or first player as drawer for the first round.
- **FR-006**: System MUST choose the secret word deterministically from the starter word list.
- **FR-007**: System MUST expose the secret word to the drawer during active play.
- **FR-008**: System MUST hide the secret word from guessers during active play.
...
- **SC-003**: One host or first-player drawer is clearly identified per started game.
