...
- Q: After host starts the game, how should clients leave the lobby? → A: All clients auto-navigate to the game screen when polling detects the room has started.
- Q: Can players join after the game has started? → A: Reject join attempts once the game has started; show a clear "game already in progress" message.
- Q: Should the manual lobby refresh button remain? → A: Keep manual refresh as an optional supplement to automatic polling.
- Q: How should the host be visually indicated in the lobby? → A: Show a `(Host)` label next to the host's name in the participant list.
- Q: What is the valid room code format? → A: Exactly 4 characters from starter alphabet: uppercase A–Z and 2–9, excluding I, O, 0, 1.
...
2. **Given** a newly created room, **When** the creator views the lobby, **Then** they are listed as a participant with a `(Host)` label next to their display name.
...
4. **Given** a player on the join screen, **When** they submit a code with invalid format (wrong length or characters outside the starter alphabet, e.g., `IO01`), **Then** they see a format error distinct from "room not found" before attempting to join.
...
6. **Given** a room whose game has already started, **When** a player submits that room's code to join, **Then** they see a clear message that the game is already in progress and are not added to the room.

---

### User Story 3 - Lobby Sync and Host-Controlled Start (Priority: P3)

Players in the lobby see the participant list update automatically without requiring manual refresh. A manual refresh control MAY remain as a fallback. Only the host can start the game, and only when at least two players are present.
...
4. **Given** a host and at least one other player in the lobby, **When** the host starts the game, **Then** the start succeeds on the server and every client in that room (host and guests) is automatically taken to the game screen when polling detects the room has left lobby status.
...
6. **Given** a player in the lobby, **When** they click manual refresh, **Then** the participant list updates immediately without disrupting automatic polling.
...
- What happens when someone tries to join after the host started? Join is rejected with a "game already in progress" message distinct from "room not found."
...
- **FR-003**: System MUST allow players to join an existing room by entering its code while the room is in lobby status.
...
- **FR-006**: System MUST reject room codes that are not exactly 4 characters or that contain characters outside the starter alphabet (uppercase A–Z and digits 2–9, excluding I, O, 0, 1), with a format error distinct from "not found."
...
- **FR-013**: System MUST automatically navigate all clients in a room to the game screen when lobby polling detects the room has started (no manual "enter game" step required).
- **FR-014**: System MUST reject join attempts when the room is no longer in lobby status, with a clear "game already in progress" message distinct from "room not found."
- **FR-015**: System SHOULD provide an optional manual lobby refresh control that supplements (does not replace) automatic polling.
- **FR-016**: System MUST display a `(Host)` label next to the host's name in the lobby participant list visible to all players in the room.

### Key Entities

- **Room**: An isolated game session identified by a 4-character code from the starter alphabet; has a host, a list of participants, and a lifecycle status beginning in lobby.
...
- **SC-003**: 100% of empty, malformed, not-found, and post-start join attempts show a specific error message (no silent failure or generic crash).
...
- **SC-006**: After a successful host start with two or more players, every participant in that room is on the game screen within 3 seconds without manual navigation.

## Assumptions

- Room codes are exactly four characters from the starter alphabet (`ABCDEFGHJKLMNPQRSTUVWXYZ23456789`); matching is case-insensitive and display normalizes to uppercase.
...
- No authentication; anyone with a room code can join that room while it is still in lobby status.

## Scope Boundaries

**In scope**: Room creation, host designation, join by code with validation, multi-room isolation, automatic lobby polling (with optional manual refresh fallback), host-only start with two-player minimum, server-authoritative start.
